...
vector<double> student_dist(int n, int nu) {
  // nu number of free degrees
  vector<double> returns;
  returns.reserve(n);
  normal_distribution<double> normal(0.0, 1.0);
  chi_squared_distribution<double> chi2(nu);

  for (int i = 0; i < n; ++i) {
    double Z = normal(gen);
    double U = chi2(gen);
    double T = Z / sqrt(U / nu);
    returns.push_back(T);
  }
  sort(returns.begin(), returns.end());
  return returns;
...
int main(int argc, const char **argv) {
  ...
  vector<double> student_returns = student_dist(N, 5);
  ...
  draw(student_returns);
  ps(mean(student_returns), standard_deviation(student_returns));
  ps("Value at risk  5%", value_at_risk(student_returns, 0.1));
  ps("Value at risk 10%", value_at_risk(student_returns, 0.05));

  student_returns = student_dist(N, 8);
  draw(student_returns);
  ps(mean(student_returns), standard_deviation(student_returns));
  ps("Value at risk  5%", value_at_risk(student_returns, 0.1));
  ps("Value at risk 10%", value_at_risk(student_returns, 0.05));
